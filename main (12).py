...
# Auto-install requests-toolbelt for real-time upload progress
try:
    from requests_toolbelt import MultipartEncoder, MultipartEncoderMonitor
except ImportError:
    subprocess.run([sys.executable, "-m", "pip", "install", "requests-toolbelt", "-q"], check=True)
    from requests_toolbelt import MultipartEncoder, MultipartEncoderMonitor
import time
...
def status_line(msg: str, end: bool = False) -> None:
    """Print a single overwriting status line (no newline unless end=True)."""
    if end:
        print(msg)
    else:
...
def upload_file(local_path: str, cloud_filename: str, uploadid_local: str, md5hash: str, partseq: int = 0, total_parts: int = 1) -> str:
    """
    Uploads a file using requests with real-time progress and speed display.
    """
    try:
        url = (
            f"{BASEURLTB.replace('www', 'c-jp')}:443/rest/2.0/pcs/superfile2"
            f"?method=upload&type=tmpfile&app_id=250528"
            f"&path={quote_plus(REMOTELOC + '/' + cloud_filename)}"
            f"&uploadid={uploadid_local}&partseq={partseq}"
        )
        file_size = os.path.getsize(local_path)
        chunk_info = f" [chunk {partseq+1}/{total_parts}]" if total_parts > 1 else ""
        ...
        def _callback(monitor):
            ...
            sys.stdout.write(
                f"\r  Progress{chunk_info}: {percent}% | "
                f"Uploaded: {_format_size(monitor.bytes_read)}/{_format_size(file_size)} | "
                f"Speed: {_format_size(speed)}/s     "
            )
            sys.stdout.flush()
        ...
        if "error_code" not in uresp:
            if uresp["md5"] == md5hash:
                # For chunks: overwrite progress, stay on same line (no \n), next chunk will overwrite
                # For single file: overwrite progress, no \n (caller handles newline)
                sys.stdout.write(
                    f"\r  ✓ Uploaded{chunk_info} in {total_time:.1f}s | "
                    f"Avg speed: {_format_size(avg_speed)}/s          "
                )
                sys.stdout.flush()
                return uresp["md5"]
            print()
            fmt.error("md5", f"MD5 hash mismatch for cloud file {cloud_filename}. Skipping...")
            return "mismatch"

        print()
        fmt.error("upload", f"File upload failed: {uresp}")
        return "failed"
    except Exception as exp_upload:
        print()
        fmt.error("upload", "File upload request failed.")
        fmt.error("upload", f"More information: {exp_upload}")
        return "failed"
...
def _process_single_file_entry(directory, file, remote_files):
    ...
    for remote_file in remote_files:
        if remote_file.get("path", "").endswith(cloud_relative):
            ...
            fmt.warning("upload", f"File {file['name']} already exists on the cloud. Skipping...")
            return
    ...
    if not os.path.exists(local_file_path):
        fmt.error("upload", f"File {rel_disp} does not exist. Skipping...")
        ERRORS = True
        return

    # Size limits
    if (vip == 1 and file['sizebytes'] >= 21474836479) or (vip == 0 and file['sizebytes'] >= 4294967296):
        fmt.error("upload", f"File {file['name']} is too big for your account type. Skipping...")
        ERRORS = True
        return

    # Print file header — this is the permanent line per file
    print(f"\nUploading: {rel_disp}  (File size: {_format_size(file['sizebytes'])})")

    # Quota check
    if SHOWQUOTA:
        _status("Checking quota...")
        quotareq = requests.get(f"{BASEURLTB}/api/quota?checkfree=1",
                                headers={"User-Agent": USERAGENT}, cookies=COOKIES, timeout=10)
    ...
    if file['sizebytes'] >= 2147483648:
        ...
        _status(f"✓ Split into {num_chunks} chunks — ready for upload.")
    else:
        _status("Calculating MD5...")
    ...
    if len(pieces) > 1:
        for idx, pi in enumerate(pieces):
            resp = upload_file(pi, cloud_relative, uploadid, md5dict[idx], idx, len(pieces))
            if resp in ("failed", "mismatch"):
                upload_failed = True
                break
        if not upload_failed:
            # End the last chunk's \r line, then print summary
            print(f"\n  ✓ All {len(pieces)} chunks uploaded successfully.")
    else:
    ...
    if not json.loads(create.text).get("errno") == 0:
        print()
        fmt.error("upload", f"File {file['name']} upload failed: {create.text}")
        ERRORS = True
        return
    ...
    if MOVEFILES:
        try:
            ...
            fmt.error("move", f"File {file['name']} could not be moved: {e}")
            ERRORS = True
            return
    ...
    if DELSRCFIL:
        try:
            ...
            fmt.error("delete", f"File {file['name']} could not be deleted: {e}")
            ERRORS = True
            return
        ...
        for item in os.listdir(TEMP_DIR):
            if item.startswith(file['name']):
                try:
                    ...
                    pass
        if deleted_chunks > 0:
            print(f"\n  ✓ Temp chunks deleted ({deleted_chunks} files).")

    # Delete source file
    if not MOVEFILES and not DELSRCFIL:
        try:
    ...
    print()  # blank line before next file
