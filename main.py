...
def _process_single_file_entry(directory, file, remote_files):
    ...
    if len(pieces) > 1:
        ...
        for idx, pi in enumerate(pieces):
            print(f"\r[CHUNK] Uploading chunk {idx + 1}/{total_chunks} of {rel_disp}...", end="", flush=True)
            resp = upload_file(pi, cloud_relative, uploadid, md5dict[idx], idx)
            if resp in ("failed", "mismatch"):
                print()  # newline after failed chunk
                upload_failed = True
                break
        if not upload_failed:
            print(f"\r[CHUNK] All {total_chunks} chunks uploaded for {rel_disp}.          ")
    else:
